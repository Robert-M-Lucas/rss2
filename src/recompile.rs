...
pub fn recompile<P: AsRef<Path>>(config: &Config, path: P) -> Result<(), String> {
    ...
    let mut path_contents = FileContents::from_path(&path)?.ok_or(format!("E45 File contents not found: {:?}", path.as_ref()))?;
    ...
    let Some(binary) = binary else {
        cprintln!("<red, bold>Failed to compile binary. Use `rss strip [file]` to remove the existing binary.</>");
        return Ok(());
    };
