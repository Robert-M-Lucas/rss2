...
pub fn edit<P: AsRef<Path>>(config: &Config, path: P) -> Result<(), String> {
    ...
    if let Some(path_contents) = path_contents {
        ...
        fs::write(&cargo_path, format!("[package]
name = \"{file_name}\"
version = \"0.1.0\"
edition = \"2024\"
...
")).map_err(|e| format!("E09 Failed to create file: {}", e))?;
        ...
        fs::write(temp_dir.path().join("src").join("main.rs"), include_str!("static/main.txt")).map_err(|e| format!("E11 Failed to create file: {}", e))?;
        let time = Instant::now() - start;
        cprintln!("<cyan>[{:?}]</>", time);
    }

    let cwd = env::current_dir().map_err(|e| format!("E51: Failed to get current working directory: {}", e))?;
    let cr_origin;
    ...
    #[cfg(unix)]
    {
        ...
        delete_cr_origin = if !cr_origin.is_file() {
            ...
            cprint!("Creating cr-orig.sh (<yellow, bold>this file will be deleted when saving!</>)... ");
            let start = Instant::now();
            fs::write(&cr_origin, &bash_script).map_err(|e| format!("E50 Failed to create cr-origin script: {}", e))?;
            let time = Instant::now() - start;
            cprintln!("<cyan>[{:?}]</>", time);
    ...
    #[cfg(windows)]
    {
        ...
        delete_cr_origin = if !cr_origin.is_file() {
            ...
            cprint!("Creating cr-orig.cmd (<yellow, bold>this file will be deleted when saving!</>)... ");
            let start = Instant::now();
            fs::write(&cr_origin, &bash_script).map_err(|e| format!("E54 Failed to create cr-origin script: {}", e))?;
            let time = Instant::now() - start;
            cprintln!("<cyan>[{:?}]</>", time);
            make_executable(&cr_origin)?;
            true
