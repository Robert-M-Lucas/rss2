...
const LAYOUT_VERSIONS: [(LayoutVersionType, &str); 1] = [
    (1, "0.2.17")
];
...
fn check_layout_version(layout_version: LayoutVersionType) -> Result<(), String> {
    if layout_version != CURRENT_LAYOUT_VERSION {
        let last_supported = LAYOUT_VERSIONS.iter().find(|(v, _)| *v == layout_version).map(|(_, v)| *v);

        if let Some(last_supported) = last_supported {
            return Err(format!(
                "E76 Unsupported layout version: {layout_version} (current is {CURRENT_LAYOUT_VERSION}).\n\
                The last rs-script release that supported this version is {last_supported}:\n\
...
impl FileContents {
    pub fn new(zipped_contents: Vec<u8>, binary_contents: Vec<u8>, triple: &str) -> Self {
        ...
        final_contents
            .extend_from_slice(&(zipped_contents.len() as LengthType).to_le_bytes()); // zipped len
    ...
    pub fn remove_binary(&mut self) {
        self.contents
            .truncate(LAYOUT_VERSION_SIZE + LENGTH_TYPE_SIZE + self.zip_length + LENGTH_TYPE_SIZE + self.triple_length);
    }

    pub fn replace_binary(&mut self, triple: &str, binary: &[u8]) {
        self.contents.truncate(LAYOUT_VERSION_SIZE + LENGTH_TYPE_SIZE + self.zip_length);
    ...
    pub fn print_stats(&self, file_name: &str) {
        cprintln!("{} <green, bold>(Layout version {}])</>:", file_name, self.layout_version);
        cprintln!(
    ...
    pub fn from_path<P: AsRef<Path>>(path: P) -> Result<Option<FileContents>, String> {
        ...
        let layout_version = LayoutVersionType::from_le_bytes(contents[cursor..cursor + LAYOUT_VERSION_SIZE].try_into().unwrap());
        ...
        let zipped_len = LengthType::from_le_bytes(contents[cursor..cursor + LENGTH_TYPE_SIZE].try_into().unwrap()) as usize;
        ...
        let triple_len = LengthType::from_le_bytes(
            contents
                [cursor..cursor + LENGTH_TYPE_SIZE]
                .try_into()
                .unwrap(),
        ) as usize;
        cursor += LENGTH_TYPE_SIZE;
    ...
    pub fn zipped_contents(&self) -> &[u8] {
        &self.contents[LAYOUT_VERSION_SIZE + LENGTH_TYPE_SIZE..LAYOUT_VERSION_SIZE + LENGTH_TYPE_SIZE + self.zip_length]
    }

    pub fn target_triple(&self) -> Cow<str> {
        String::from_utf8_lossy(
            &self.contents[LAYOUT_VERSION_SIZE + LENGTH_TYPE_SIZE + self.zip_length + LENGTH_TYPE_SIZE
                ..LAYOUT_VERSION_SIZE + LENGTH_TYPE_SIZE + self.zip_length + LENGTH_TYPE_SIZE + self.triple_length],
        )
    }

    pub fn bin_contents(&self) -> &[u8] {
        &self.contents[LAYOUT_VERSION_SIZE + LENGTH_TYPE_SIZE + self.zip_length + LENGTH_TYPE_SIZE + self.triple_length..]
    }
