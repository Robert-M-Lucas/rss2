...
const LAYOUT_VERSIONS: [(LayoutVersionType, &str); 1] = [(1, "0.2.21")];
#[allow(dead_code)]
    ...
    if layout_version != CURRENT_LAYOUT_VERSION {
        ...
        return if let Some(last_supported) = last_supported {
            ...
            Err(format!(
                ...
                - Generated by a newer version of rs-script (current is {RS_SCRIPT_VERSION})\n\
                - Generated by a version before 0.2.17 (before layout versioning)\n\
                - Corrupted\n\
                If the file was created by a version before 0.2.17, extract the project in that version and then pack it in the latest version.",
            ))
        };
    }
    Ok(())
